...
(defmethod handle-message* :leave [{:keys [group-id user-id] :as message}]
  (if-let [current-team (db/get-current-team group-id user-id)]
    (do
      (db/remove-from-team! group-id current-team user-id)
